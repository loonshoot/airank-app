...
const PERMISSION_OPTIONS = [
  ...
  { value: "query:prompts", label: "View Prompts" },
  ...
  { value: "mutation:createPrompt", label: "Create Prompts" },
  { value: "mutation:updatePrompt", label: "Update Prompts" },
  { value: "mutation:deletePrompt", label: "Delete Prompts" },
];
...
export default function TeamPage({ params }) {
  ...
  // GraphQL client (standard pattern)
  ...
  // Fetch members using standard pattern
  const fetchMembers = useCallback(async () => {
    if (!workspaceSlug || !graphqlClient) return;
    ...
    try {
      ...
      if (result.data) {
        ...
        console.log('TeamPage - members fetched:', result.data);
      } else if (result.error) {
        console.error('TeamPage - error fetching members:', result.error);
        setMembersError(result.error);
      }
    } catch (error) {
      console.error('TeamPage - GraphQL error:', error);
      setMembersError({ message: error.message });
    }

    setIsMembersLoading(false);
  }, [graphqlClient, workspaceSlug]);

  // Fetch members on mount and when workspace changes
  useEffect(() => {
    fetchMembers();
  }, [fetchMembers]);
  ...
  // Invite using GraphQL mutation (standard pattern)
  const invite = async () => {
  ...
  // Remove member using GraphQL mutation (standard pattern)
  ...
  // Update permissions using GraphQL mutation (standard pattern)
  ...
  return (
    <AccountLayout routerType="app">
      <Meta title={`AI Rank - ${workspace?.name || 'Workspace'} | Team Settings`} />
      <Content.Title
